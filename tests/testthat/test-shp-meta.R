...
test_that("shp_meta works", {
  expect_identical(nrow(shp_file_meta(character(0))), 0L)
  expect_identical(nrow(shp_file_meta(shp_example("mexico/cities.shp"))), 1L)
  expect_identical(nrow(shp_file_meta(shp_example(rep("mexico/cities.shp", 2)))), 2L)

  expect_identical(nrow(shp_file_meta(shp_example_all())), length(shp_example_all()))

  expect_error(shp_file_meta("does_not_exist.shp"), "Unable to open")
})

test_that("shp_geometry works", {
  cities <- shp_geometry_meta(shp_example("mexico/cities.shp"))
  expect_is(cities, "data.frame")
  expect_identical(nrow(cities), shp_file_meta(shp_example("mexico/cities.shp"))$n_features)
  ...
  all_metas <- shp_file_meta(shp_example_all())
